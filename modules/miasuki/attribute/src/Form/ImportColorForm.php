...
/**
 * Class ImportColorForm.
 */
class ImportColorForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['csv_file'] = [
      ...
      '#title' => $this->t('Attribute Color CSV File'),
      '#weight' => '0',
    ];
    $form['submit'] = [
      '#type' => 'submit',
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($all_data as $each_data) {
      ...
      $db_fields = array(
        ...
        'nav_color' => $each_data['nav_color'],
      );
      $query_check = $connection->select('miasuki_attribute_color', 'mac')
          ->condition('magento_id', $db_fields['magento_id'])
          ->fields('mac');
      $record = $query_check->execute()->fetchAssoc();
      if (!empty($record['id'])) {
        $query_update = $connection->update('miasuki_attribute_color')
          ->fields($db_fields)
          ->condition('id', $record['id'])
          ->execute();
      }else{
        $query_insert = $connection->insert('miasuki_attribute_color')
          ->fields($db_fields)
          ->execute();
      }
    }
